...
<div class="container-fluid py-4">
    <div class="card p-4">
        ...
        <div class="table-responsive">
            ...
            <table id="operTable" class="table table-bordered table-hover table-striped"
...
$(document).ready(function() {
    var table = $('#operTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: './oper_data_server.php',
            data: function(d) {
...
function clearFilters() {
    ...
    $('#operTable').DataTable().draw();
}

function exportToExcel() {
    var dt = $('#operTable').DataTable();
    var searchVal = dt.search();
    var params = new URLSearchParams({
        search:           searchVal,
        filterRefCode:    $('#filterRefCode').val(),
        filterCode30:     $('#filterCode30').val(),
        filterEmptyRef:   $('#filterEmptyRef').is(':checked') ? 'true' : 'false',
        filterEmptyCode30: $('#filterEmptyCode30').is(':checked') ? 'true' : 'false'
    });
    window.location.href = 'export_oper_excel.php?' + params.toString();
}

function openStatusModal(code) {
    $.ajax({
        url: 'get_item_oper.php',
        type: 'POST',
        data: {
            code: code
        },
